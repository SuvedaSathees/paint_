...
async function renderPaintCan() {
  const inputPath = "public/akshara-paint-can-empty-backup.png";
  const outputPath = "public/akshara-paint-can.png";

  // SVG overlay exactly 1200x1408
  const svg = `
<svg width="1200" height="1408" viewBox="0 0 1200 1408" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <!-- Liquid paint pool radial gradient -->
    <radialGradient id="paintPool" cx="46%" cy="40%" r="58%" fx="42%" fy="35%">
      <stop offset="0%" stop-color="#ffb833" />
      <stop offset="25%" stop-color="#f58220" />
      <stop offset="65%" stop-color="#e66005" />
      <stop offset="92%" stop-color="#cc4a00" />
      <stop offset="100%" stop-color="#8a2e00" />
    </radialGradient>
    ...
    <!-- Drip 3D volume gradient -->
    <linearGradient id="dripVol" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#ff9d2b" />
      <stop offset="30%" stop-color="#ffbe42" />
      <stop offset="70%" stop-color="#e66405" />
      <stop offset="100%" stop-color="#b84500" />
    </linearGradient>

    <!-- Drop highlight specular -->
    <radialGradient id="dropHighlight" cx="35%" cy="30%" r="60%">
      <stop offset="0%" stop-color="#ffffff" stop-opacity="0.9" />
      <stop offset="35%" stop-color="#ffbe42" stop-opacity="0.8" />
      <stop offset="75%" stop-color="#ea580c" stop-opacity="1" />
      <stop offset="100%" stop-color="#9a3412" stop-opacity="1" />
    </radialGradient>
  ...
  <g id="paint-pool">
    <!-- Main pool filling the inner rim -->
    <ellipse cx="593" cy="138" rx="368" ry="34" fill="url(#paintPool)" />

    <!-- Liquid surface gloss highlight crescent -->
    <path d="M 320,136 Q 590,158 860,136 Q 590,147 320,136 Z" fill="url(#surfaceGloss)" opacity="0.85" />
    
    <!-- Secondary delicate rim specular reflection -->
    <path d="M 280,130 Q 590,165 900,130 Q 590,162 280,130 Z" fill="#ffffff" opacity="0.4" />
  </g>

  <!-- 2. WET PAINT DRIPPING OUTSIDE OVER THE FRONT RIM -->
  <g id="paint-drips" filter="url(#dripShadow)">
    <!-- Organic continuous paint overflow apron with smooth drips -->
    <path d="
      M 395,166
...
`;
  ...
  console.log("Vector-perfect paint can rendered successfully with inside paint, drips and droplets!");
}

renderPaintCan().catch(console.error);
